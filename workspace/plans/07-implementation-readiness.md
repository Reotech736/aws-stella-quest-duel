...
- Room、Active Context、Join Guard、Game State、Event、Requestの保存型
- 各アクセスパターンのパーティションキーとソートキー
- 時系列順を維持するイベント連番キー
- 完全なゲーム状態の保存・復元
- 終了結果のプレイヤーIDとCognitoユーザーIDの相互変換
- `undefined` を保存しないDynamoDB DocumentClient
- 強整合・結果整合を指定できるゲーム状態取得
- `version` を条件にしたゲーム状態の楽観的ロック
- ゲーム状態、イベント、冪等性記録のトランザクション書き込み
- DynamoDBの条件競合を永続化層の競合エラーへ変換
- Roomの強整合・結果整合取得
- Active ContextとJoin Guardの強整合取得
- ルーム、作成者コンテキスト、冪等性記録の作成トランザクション
- ルーム参加、参加者コンテキスト、Join Guard解除のトランザクション
- ルーム、両プレイヤーのコンテキスト、ゲーム状態、開始イベント、冪等性記録のゲーム開始トランザクション
- Join Guardの15分集計、5回失敗後15分ブロック、24時間TTL計算
- Join Guardの読み取り状態を条件にした競合検知付き保存
- USER、ROOM、GAMEスコープの冪等性記録の強整合取得
- 同一リクエストの再送と異なる内容でのキー再利用の判定
- 開始前退出と待機期限切れのトランザクション
- ゲーム終了時のRoom、Active Contextを含む更新
...
- CloudWatch Logs
- CORS、スロットリング、TTL
- `Project`、`Environment`、`ManagedBy`の統一タグ
...
- [x] 対象AWSアカウントとリージョン確認
- [x] アカウント全体Budget作成
- [x] SAMデプロイ
- [x] Cognito利用者作成
- [x] ローカルフロントエンドから2ユーザーでスモークテスト
- [ ] UI改善
- [ ] プロジェクト専用BudgetのIaCデプロイ
- [ ] Amplify Hosting接続
- [ ] 共有パスワード
- [ ] Amplify URLへのCORSオリジン更新
- [ ] 公開URLから2ユーザーでスモークテスト

現在の詳細は`workspace/plans/08-aws-deployment.md`、今後の作業は`workspace/plans/09-ui-improvement.md`と`workspace/plans/10-amplify-beta-release.md`を参照します。
