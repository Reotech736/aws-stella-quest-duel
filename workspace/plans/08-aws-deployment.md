...
- DynamoDB、Cognito、HTTP API、Lambda、CloudWatch Logs: 作成済み
- Cognitoテストユーザー: 管理者作成済み
- 実環境スモークテスト: ローカルフロントエンドから2ユーザーでゲーム終了まで確認済み
- APIのCORS許可元: `http://localhost:5173`
- Amplify Hosting: 未作成
- アカウント全体Budget: 作成済み
- プロジェクト専用Budget: IaC作成済み、未デプロイ

初回変更セットと、その後のLambda形式・IAM権限修正の変更セットは実行済みです。今後もSAM管理リソースの変更は、変更セットの内容を確認してから反映します。
...
プロジェクト専用Budgetはアプリケーションスタックと分離し、`infrastructure/cost-budget.yaml`で管理します。通知先、Amplify HostingのGit接続、共有パスワード、Cognito利用者作成など、外部情報や管理者操作を伴う項目はSAMアプリケーションスタックへ含めません。
...
DynamoDB権限は、このスタックのゲームテーブルだけを対象に、Lambdaの責務ごとに分けます。

| Lambda | 許可する操作 |
| --- | --- |
| Context/Room | `GetItem`、`PutItem`、`DeleteItem`、`TransactWriteItems` |
| Game Query | `GetItem` |
| Game Command | `GetItem`、`PutItem`、`DeleteItem`、`TransactWriteItems` |
...
Billing and Cost Managementの「コスト配分タグ」で`Project`と`Environment`を有効化し、Cost Explorerへ反映されたことを確認してからプロジェクト専用Budgetをデプロイします。

## 変更セットの読み方

変更セットのOperationが`Add`の場合は新規作成です。初回変更セットは23件の新規追加だけで、確認後に実行しました。
...
- リージョンは`ap-northeast-1`、環境名は`dev`
- IAMロールは対象DynamoDBテーブルだけを操作する
- DynamoDBとCognitoはスタック削除時に保持される
- LambdaはNode.js 24で読み込めるCommonJS成果物として動作する
- 2ユーザーがルームを作成・参加し、ゲーム終了まで進行できる

## 残作業

1. `workspace/plans/09-ui-improvement.md`の完了条件を満たす。
2. Amplify Hostingで限定ベータ環境を作成する。
3. Amplify URLを`AllowedOrigin`へ指定したSAM変更セットを作成し、確認後に実行する。
4. 公開URLから2ユーザーのスモークテストを実施する。
5. コスト配分タグの反映確認後、`infrastructure/cost-budget.yaml`を別スタックとしてデプロイする。
