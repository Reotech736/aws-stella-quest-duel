...
- 限定ベータ中はカスタムドメインを使用しない
- 正式公開時のゲームURLは`https://stella-quest-duel.reotech736.com`とする
- `reotech736.com`の既存ブログは維持し、ゲーム紹介ページはブログ側でユーザーが作成する
...
フェーズ10の完了までは既存のAmplify URLを使用します。カスタムドメイン、DNS、SSL証明書、カスタムドメイン向けCORSはフェーズ11で扱い、このフェーズでは変更しません。
...
完了後は`workspace/plans/11-custom-domain-release.md`へ進みます。
