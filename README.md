...
ゲームドメイン、DynamoDB永続化、Context/Room API、Game Query / Command API、Cognito認証クライアント、主要画面、SAMテンプレートまでローカル実装済みです。

実AWSへのデプロイ、Amplify Hosting、Budget、Cognito利用者作成、実環境の2ユーザースモークテストは未実施です。これらは対象アカウントとリージョンを確認し、明示的な承認を得てから行います。
