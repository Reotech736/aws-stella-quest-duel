...
ゲームドメインは初期化から終了・公開状態まで実装済みです。DynamoDBアクセスでは、DocumentClient、ゲーム状態取得、楽観的ロック、イベント・冪等性記録を含むトランザクションまで実装しています。次はRoom、Active Context、Join GuardのRepositoryと、ルーム参加・開始時の更新フローを実装します。
